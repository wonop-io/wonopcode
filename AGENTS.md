...
### Example: Correct Feature Development Flow

```
User: "I want to add a login feature"

Agent: I'll help you implement a login feature. Let me first create a feature 
       and gather requirements systematically.
       [uses ace_create_feature]

       [uses ace_elicit to get suggested questions]

       Let me start by understanding who will use this feature.
       Who are the users that will log in? (e.g., customers, admins, both?)

User: "Both customers and admins"

Agent: [uses ace_record_response with category="actors"]
       [uses ace_elicit for next question]

       Got it. What authentication methods should be supported - just 
       password, or also OAuth and SSO?

User: "Password and Google OAuth"

Agent: [uses ace_record_response with category="operations"]
       [continues asking questions one at a time...]
       [after 5-10 questions covering key categories:]

       [uses ace_complete_elicitation]

       I now have a good understanding. Let me document the formal requirements.
       [uses ace_create_artifact for requirements]

       I've created the following requirement document:
       [shows REQ-xxx content]

       Please review these requirements - do they capture what you need?
       Reply with "approved" to proceed, or let me know what to change.

User: "Approved"

Agent: [uses ace_submit_checkpoint action="approve"]
       [continues to use case modeling phase]
```
