...
// @ace:implements COMP-T90R60-374
...
impl OpenAIProvider {
    ...
    /// Check if this model is a reasoning model (o-series or GPT-5.x).
    ...
    fn convert_messages(
        ...
        for msg in messages {
            ...
            if msg.role == Role::Tool {
                ...
                // For assistant messages, always include even if content is null
                // because we may have tool_calls
                let tool_calls = convert_tool_calls(&msg.content);

                // Only add the message if we have content or tool calls
                if !content.is_null() || tool_calls.is_some() {
                    let mut message = if content.is_null() {
                        // OpenAI requires content to be a string, even if empty
                        json!({
                            "role": role,
                            "content": ""
                        })
...
struct ChatRequest {
    model: String,
    messages: Vec<Value>,
    /// Used for most models (gpt-4o, gpt-4, etc.)
    #[serde(skip_serializing_if = "Option::is_none")]
    max_tokens: Option<u32>,
    /// Used for reasoning models (o1, o3, etc.)
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl LanguageModel for OpenAIProvider {
    async fn generate(
        ...
        // Reasoning models (o1, o3) have different API requirements
        let is_reasoning = self.is_reasoning_model();

        let request = ChatRequest {
            model: self.model.id.clone(),
            messages: Self::convert_messages(&messages, options.system.as_deref(), is_reasoning),
            ...
            top_p: if is_reasoning { None } else { options.top_p },
        ...
        debug!(model = %self.model.id, "Sending OpenAI request");
        ...
        tracing::info!(status = %response.status(), "OpenAI API response received");
            ...
            while let Some(line) = lines.next_line().await? {
                ...
                for choice in &chunk.choices {
                    ...
                    if let Some(reason) = &choice.finish_reason {
                        ...
                        // Get usage from this chunk if available
                ...
                // Check for usage in a chunk without choices (OpenAI sometimes sends usage separately)
...
mod tests {
    ...
    fn test_convert_messages() {
        ...
        // Test with non-reasoning model (uses "system" role)
        let converted = OpenAIProvider::convert_messages(&messages, Some("You are helpful"), false);
    ...
    #[test]
    fn test_convert_messages_reasoning_model() {
        let messages = vec![Message::user("Hello, world!")];

        // Test with reasoning model (uses "developer" role)
        let converted = OpenAIProvider::convert_messages(&messages, Some("You are helpful"), true);
