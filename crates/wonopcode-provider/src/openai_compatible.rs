...
//! Examples: Ollama, LM Studio, vLLM, etc.
...
impl OpenAICompatibleProvider {
    ...
    pub fn new(config: OpenAICompatibleConfig) -> ProviderResult<Self> {
...
impl LanguageModel for OpenAICompatibleProvider {
    async fn generate(
        ...
        let request = ChatRequest {
            ...
            stream: true,
            // Some providers don't support stream_options, so only include if using tools
            stream_options: if options.tools.is_empty() {
                None
            } else {
                Some(StreamOptions {
                    include_usage: true,
        ...
        debug!(model = %self.model.id, base_url = %self.base_url, "Sending OpenAI-compatible request");
...
mod tests {
    ...
    fn test_convert_messages() {
        ...
        let converted =
            OpenAICompatibleProvider::convert_messages(&messages, Some("You are helpful"));
    ...
    fn test_convert_tools() {
        ...
        let converted = OpenAICompatibleProvider::convert_tools(&tools);
